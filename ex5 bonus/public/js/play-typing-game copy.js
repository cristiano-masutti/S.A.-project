...
function pickRandomChallenge() {
    ...
    /*** Quiz 3 ***/
    let i = Math.floor(Math.random() * challenges.length);
    document.querySelector(".id span").innerHTML = i;
    return [challenges[i], i];
}
...
function gameOver() {
    ...
    window.location.assign(`http://localhost:8888/high_scores/game_over?player=${player}&score=${document.querySelector('.score span').innerHTML}&time=${document.querySelector('.time span').innerHTML}&game=typing&challenge=${challenge}`);
}
