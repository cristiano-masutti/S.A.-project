...
function checkUrl(table, scoreO, scoreX, tie) {
    if (table.length != 9) {
        return true;
    }
    if (scoreO<0 ||scoreX<0 || tie<0 ) {
        return true;
    }
 ...
 function createTableAi(table, player, urlParameters) {
    let board = "";
    for (let i = 0; i < table.length; i++) {
        if (table[i] === '_' &&  !tictactoe.win(table)) {
            newBoard = createNewBoard(table, i, player);
            if(urlNotPath.length == 1) {
                board += `<b><a href="/tic-tac-toe-ai/${newBoard}">${table[i]}</a></b>`;
            } else {
                board += `<b><a href="/tic-tac-toe-ai/${newBoard}?${urlParameters}">${table[i]}</a></b>`;
            }
        } else {
            board += `<b>${table[i]}</b>`;
        }
 ...
 function html(action, scoreX, scoreO, tie, message, urlParameters, board) {
    return `
    <html>
        <head>
        ...
        <body class="${action}">
            <main>
                ...
                <nav>
                    <a href="http://localhost:8888/a">Reset</a>
                    <a href="http://localhost:8888/tic-tac-toe-ai/_________?${urlParameters}">Continue</a>
                    <a href="http://localhost:8888/tic-tac-toe/_________?${urlParameters}">Play against player</a>
                </nav>
                <div class="grid">
                ${board}
                </div>
 ...
 function checkWin(table) {
    if (tictactoe.win(table)) {
        i = 0;
        console.log(tictactoe.getWinner(table));
        if (tictactoe.getWinner(table) === O.toUpperCase()) { 
            scoreO++;
...
//Parse Request URL
    ...
    urlParameters = newUrl.searchParams;
    scoreX = parseInt(urlParameters.get("scoreX")) || 0;
    scoreO = parseInt(urlParameters.get("scoreO")) || 0;
    tie = parseInt(urlParameters.get("ties")) || 0;

// //Check if the URL path begins with /tic-tac-toe/ (Tasks 3-6)
    if (url.indexOf("/tic-tac-toe-ai/")===0) {
        if (player === X) {
            player = O;
        } else {
            player = X;
        }
        let table = urlPaths[2];
        if (checkUrl(table, scoreO, scoreX, tie)) {
            ...
            checkWin(table);
            if (player == X && message=="") {
                let choice = ai.great_ai(table);
                console.log(choice);
                console.log(table);
                table = createNewBoard(table, choice, player);
                console.log(table);
                player = O;
                checkWin(table);
                let board=createTableAi(table, player, urlParameters);
                response.writeHead(200, {'Content-Type':'text/html'});
                response.end(html(action, scoreX, scoreO, tie, message, urlParameters, board));
            } else {
                let board=createTableAi(table, player, urlParameters);
                response.writeHead(200, {'Content-Type':'text/html'});
                response.end(html(action, scoreX, scoreO, tie, message, urlParameters, board));
            ...
            <html>
                ...
                <body class="${action}">
                    <main>
                        ...
                        <p class="message">${message}</p>
                        <nav>
                            <a href="http://localhost:8888/p">Reset</a>
                            <a href="http://localhost:8888/tic-tac-toe/_________?${urlParameters}">Continue</a>
                            <a href="http://localhost:8888/tic-tac-toe-ai/_________?/${urlParameters}">Play against AI</a>
                        </nav>
                        <div class="grid">
                        ${board}
                        </div>
...
// //Check if the URL path begins with an existing filename (inside the site_path folder) (Task 3)
        ...
        response.writeHead(302, {"Location" : "/tic-tac-toe-ai/_________?scoreX=0&scoreO=0&ties=0", "Content-Type" : "text/html"});
        response.end();
    }
